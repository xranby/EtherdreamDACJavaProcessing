...
public class Point {
  ...
  Point(int x, int y, int r, int g, int b) {
     this.x=x;this.y=(int)(y*0.5);this.r=r;this.g=g;this.b=b;           
  }
}
...
DACPoint[] getDACPointsAdjusted(Point[] points) {
    ...
    for(Point p: points){
      ...
      //lerp adjusted
      result = concatPoints(result,getDACPointsLerpAdjusted(last,p,50,0.117));
...
DACPoint[] getDACPoints() {
  p.clear();
  p.add( new Point(0,mx,0,0,0) ); // return home
  ...
  for(Integer k : keys){
    ...
    if(value!=0){
      ...
      colorMode(RGB,65535);
      float scale = 100.0f/value;
      p.add( new Point(mx-(max/12*(k%12)),(int)(0+(mi*scale)),(int)red(c),(int)green(c),(int)blue(c)) );
    }
  }
...
void draw() {
  ...
  for(Integer k : keys){
    ...
    if(value!=0){
      ...
      strokeWeight(value);
      float scale = (100.0f/value);
      line(width/2,0,width/12*(k%12),height-(((8*12)+(12*(k/12)))*scale));
    }
  }
}

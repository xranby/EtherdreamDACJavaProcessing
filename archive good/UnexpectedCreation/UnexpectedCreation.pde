...
DACPoint[] getDACPoints() {
    ...
    int y = mx;
    for (int i = 0; i < 50; i++) {
        x = (int)lerp(x,mi/(lastK/20.0),pp);
        y = (int)lerp(y,mx/(lastK/20.0),pp);
    
        result[i] = new DACPoint(x, y,
            65535,     0,     0);
    ...
    for (int i = 50; i < 100; i++) {
        x = (int)lerp(x,mi/(lastK/20.0),ppp);
        y = (int)lerp(y,mi/(lastK/20.0),ppp);
...
void draw() {
  ...
  pp=(float)mouseY/((float)height);
  ppp=(float)mouseX/((float)width);
  
  textFont(f);
  textAlign(CENTER);
  text("pp="+pp,width/2,60); 
  text("ppp="+ppp,width/2,120); 

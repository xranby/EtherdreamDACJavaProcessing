...
DACPoint[] getDACPoints(Point[] parray) {
    DACPoint[] result = new DACPoint[parray.length];
    int i=0;
    for(Point p: parray){
      result[i] = new DACPoint(p.x, p.y,
            p.r,     p.g,     p.b);
...
DACPoint[] getDACPointsAdjusted(Point[] points) {
    ...
    for(Point p: points){
      ...
      // delay adjusted
      //result = concatPoints(result,getDACPointsDelayAdjusted(p,25));
     
      //result = concatPoints(result,getDACPointsLinearAdjusted(last,p,25));
     
      //lerp adjusted
      result = concatPoints(result,getDACPointsLerpAdjusted(last,p,25,0.14));
      //result = concatPoints(result,getDACPointsLerpAdjusted(last,p,15,d));
      
     
      /*
      if((l<500)&&(l!=0)){
          result = concatPoints(result,getDACPointsLerpAdjusted(last,p,3,0.14));
      } else if((l<2000)&&(l!=0)){
          result = concatPoints(result,getDACPointsLerpAdjusted(last,p,7,0.14));
      } else if((l<7000)&&(l!=0)){
          result = concatPoints(result,getDACPointsLerpAdjusted(last,p,20,0.14));
      } else {
          result = concatPoints(result,getDACPointsLerpAdjusted(last,p,30,0.14));
      }*/
...
DACPoint[] getDACPoints() {
    ...
    p[9] = new Point(mi/4,mx/4,0,0,0);   // blank line
   
    p[10] = new Point(mx/4,mx/4,on,0,0);
    p[11] = new Point(mx/4,mi/4,0,on,0);
    p[12] = new Point(mi/4,mi/4,on,on,0);
    p[13] = new Point(mi/4,mx/4,0,0,on);
    
    p[14] = new Point(mi/10,mx/10,0,0,0);   // blank line
   
    p[15] = new Point(mx/10,mx/10,on,0,0);
    p[16] = new Point(mx/10,mi/10,0,on,0);
    p[17] = new Point(mi/10,mi/10,on,on,0);
    p[18] = new Point(mi/10,mx/10,0,0,on);
    
    p[19] = new Point(mi/20,mx/20,0,0,0);   // blank line
   
    p[20] = new Point(mx/20,mx/20,on,0,0);
    p[21] = new Point(mx/20,mi/20,0,on,0);
    p[22] = new Point(mi/20,mi/20,on,on,0);
    p[23] = new Point(mi/20,mx/20,0,0,on);
    
    p[24] = new Point(mi/40,mx/40,0,0,0);   // blank line
   
    p[25] = new Point(mx/40,mx/40,on,0,0);
    p[26] = new Point(mx/40,mi/40,0,on,0);
    p[27] = new Point(mi/40,mi/40,on,on,0);
    p[28] = new Point(mi/40,mx/40,0,0,on);
    
    p[29] = new Point(mi/80,mx/80,0,0,0);   // blank line
   
    p[30] = new Point(mx/80,mx/80,on,0,0);
    p[31] = new Point(mx/80,mi/80,0,on,0);
    p[32] = new Point(mi/80,mi/80,on,on,0);
    p[33] = new Point(mi/80,mx/80,0,0,on);
    
    p[34] = new Point(mi,mx,0,0,0);      // blank line
    
    p[35] = new Point(mi,mx,0,0,0);      // blank line
    
    //return pointsMinimum(getDACPoints(p),600);

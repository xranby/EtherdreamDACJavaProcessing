...
if(f<55){
  ...
  doom=freq_buffer_index;
  freq_buffer[freq_buffer_index] = level*1.6*laserMax;
 ...
 /* 
  ...
  int by = (int)(mx2*sin((våg/2.0)/5.0))-mx2/2; 
  p.add( new Point(mi,by/4)); // blank
  
  for(int i = 1; i < 25; i++){
    color c = numberToColor(min(1+(int)((i+mouseOffset)%25)/(25/6),6));
   ...
   int y = (int)(mx2*sin((våg/3.0+i+doom)/2.0))-mx2/2; 
  ...
  offset+=freq_buffer[(i+mouseOffset)%25];
  //offset+=freq_buffer[i];
     
    p.add( new Point((y/4)+offset,(int)(x*1.5),r,g,b)); 
